...
contract BokerDapp is BokerManaged {
    ...
    */
    function dappSet(address dappAddr, uint256 dappType, uint256 userCount, uint256 monthlySales) external onlyContract {
        BokerDappData(contractAddress(ContractDappData)).dappSet(dappAddr, dappType, userCount, monthlySales);
    }

    function dappGetAdresses() external view returns (address[] memory addrDapps) {
        return BokerDappData(contractAddress(ContractDappData)).dappGetAdresses();
    }
}
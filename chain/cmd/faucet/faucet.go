...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/Bokerchain/Boker/chain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			tx := types.NewTransaction(protocol.Binary, f.nonce+uint64(len(f.reqs)), address, amount, big.NewInt(21000), f.price, nil)

...
const (
	...
	badBlockLimit       = 10
	BlockChainVersion   = 3 //BlockChainVersion 确保不兼容的数据库从头开始强制重新同步
)
...
func NewBlockChain(chainDb ethdb.Database,
	...
	log.Info("SetValidator and SetProcessor")
	...
	log.Info("Check the current state")
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i := 1; i < len(chain); i++ {
		...
		//判断区块序号必须连续，并且区块的父块Hash和上一个区块的Hash一致
		if chain[i].NumberU64() != chain[i-1].NumberU64()+1 || chain[i].ParentHash() != chain[i-1].Hash() {
			...
			return 0,
				nil,
				nil,
				fmt.Errorf("non contiguous insert: item %d is #%d [%x…], item %d is #%d [%x…] (parent [%x…])", i-1, chain[i-1].NumberU64(),
					chain[i-1].Hash().Bytes()[:4],
					i,
					chain[i].NumberU64(),
					chain[i].Hash().Bytes()[:4],

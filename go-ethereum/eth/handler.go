...
import (
	...
	"github.com/boker/go-ethereum/eth/fetcher"
	...
	"github.com/boker/go-ethereum/p2p/discover"
...
type ProtocolManager struct {
	...
	downloader *downloader.Downloader //负责所有向相邻个体主动发起的同步流程
	fetcher    *fetcher.Fetcher       //累积所有其他个体发送来的有关新数据的宣布消息，并在自身对照后，安排相应的获取请求
	peers      *peerSet               //缓存相邻个体列表
...
// handle 用来管理eth peer的生命周期而调用的回调。 当函数终止时，节点将断开连接
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	//根据内容处理消息
	switch {
	case msg.Code == StatusMsg: //StatusMsg应该在HandleShake阶段接收到。 经过了HandleShake之后是不应该接收到这种消息的
		return errResp(ErrExtraStatusMsg, "uncontrolled status message")

	case msg.Code == GetBlockHeadersMsg: //接收到请求区块头的消息， 会根据请求返回区块头信息
	...
	case msg.Code == GetBlockBodiesMsg: //Block Body的请求这个比较简单。 从blockchain里面获取body返回就行
	...
	case p.version >= eth63 && msg.Code == GetReceiptsMsg: //请求收据
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err

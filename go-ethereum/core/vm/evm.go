...
func (evm *EVM) Call(caller ContractRef,
	addr common.Address,
	input []byte,
	gas uint64,
...
func (evm *EVM) BaseCall(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {

	if evm.vmConfig.NoRecursion && evm.depth > 0 {
		return nil, gas, nil
	}

	//判断合约调用是否大于最大的执行限制 1024
	if evm.depth > int(params.CallCreateDepth) {
		return nil, gas, ErrDepth
	}
...
func (evm *EVM) BaseCreate(caller ContractRef, code []byte, gas uint64, value *big.Int) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
	...
	if err == nil && !maxCodeSizeExceeded {
		...
		//对结果进行存储还需要消耗gas
		createDataGas := uint64(len(ret)) * params.CreateBaseContractGas
		if contract.UseGas(createDataGas) {
			evm.StateDB.SetCode(contractAddr, ret)
		} else {
			err = ErrCodeStoreOutOfGas

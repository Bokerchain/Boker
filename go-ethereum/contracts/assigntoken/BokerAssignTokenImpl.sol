...
contract BokerAssignTokenImpl is AccessControl, BokerAssignTokenDefine, BokerAssignTokenEventHandler, Log{
    ...
    */
    ...
    function _isCurrentCycleEnd() private view returns (bool) {
        if(data.assignCycleBegin() + data.assignCyclePeriod() <= now) {
            return true;
        }
    ...
    */
    function checkAssignToken() onlyCLevel external view returns (bool) {
        if(_isCurrentCycleEnd()) {
            return true;
        }
    ...
    */
    function assignToken() onlyCLevel external {      
        ...
        data.setAssignCycleBegin(data.assignCycleBegin() + data.assignCyclePeriod());
    }    
}
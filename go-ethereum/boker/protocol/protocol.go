...
const (
	...
	ProducerInterval    = int64(2)     //打包时间间隔（秒）
	TokenNoderInterval  = int64(60)    //分配通证时间间隔(秒)
...
var (
	ErrRpcCmd = errors.New("Prohibited RPC Cmd")
)
...
var (
	...
	ErrInvalidTokenTime     = errors.New("invalid time to assign token noder")
	ErrInvalidCoinbase      = errors.New("invalid current mining coinbase")                 //当前挖矿账号错误
	ErrInvalidSystem        = errors.New("invalid current system votes")                    //当前系统的投票合约出错
	...
	ErrEthereumRuning       = errors.New("invalid Ethereum is`t Runing")                    //以太坊没有运行
...
type BokerBackendProto struct {
	BaseHash     common.Hash `json:"BaseRoot"        gencodec:"required"`
	ContractHash common.Hash `json:"ContractRoot"    gencodec:"required"`
}

func (p *BokerBackendProto) Root() (h common.Hash) {

	hw := sha3.NewKeccak256()
	rlp.Encode(hw, p.BaseHash)
	rlp.Encode(hw, p.ContractHash)
	hw.Sum(h[:0])
	return h
}

func ToBokerProto(baseAddr common.Hash, contractAddr common.Hash) *BokerBackendProto {

...
import (
	...
	"github.com/boker/go-ethereum/ethstats"
	"github.com/boker/go-ethereum/les"
	...
	"github.com/boker/go-ethereum/p2p/discv5"
	"github.com/boker/go-ethereum/p2p/nat"
	"github.com/boker/go-ethereum/p2p/netutil"
...
/*func setValidator(ctx *cli.Context, ks *keystore.KeyStore, cfg *eth.Config) {
	...
	if ctx.GlobalIsSet(ValidatorFlag.Name) {
		...
		if err != nil {
			...
			log.Error("Option %q: %v", ValidatorFlag.Name, err)
		}
...
func setCoinbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *eth.Config) {
	...
	if ctx.GlobalIsSet(CoinbaseFlag.Name) {
		...
		if err != nil {
			log.Error("Option %q: %v", CoinbaseFlag.Name, err)
		}
	...
	if (cfg.Coinbase == common.Address{}) {
		if len(accounts) > 0 {
			...
			log.Error("No coinbase set and no accounts found as default")
		}
	}
}
...
// RegisterEthService 添加一个以太坊客户端节点

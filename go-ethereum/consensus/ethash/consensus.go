...
func (ethash *Ethash) Finalize(chain consensus.ChainReader,
	...
	//生成MPT根(MPT全称Merkle Patricia Trie 是以太坊用来存储状态信息的一种数据结构，这棵树的根需要存储到区块头中)
...
//计算报酬，注意由于采用的Dpos因此不会出现叔块的可能性，而最新版的以太坊中此处是按照循环来进行叔块报酬计算的
func AccumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {

	//Bobby的出块奖励数量（11Bobby）
	blockReward := big.NewInt(1)
	blockReward.Mul(BobbyUnit, BobbyMultiple)
	...
	blockTransfer.Mul(TransferUnit, TransferMultiple)
